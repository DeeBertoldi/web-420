...
// Chapter 6 - Login Tests

describe("Chapter 6: API Tests", () => {

  it("return a 200 status with 'Authentication successful' message", async () => {
  ...
  it("401 status code with 'Unauthorized' message when logging in with incorrect credentials", async () => {
    const res = await request(app).post("/api/login").send({
      email: "harry@hogwarts.edu",
  ...
  it("400 status code with 'Bad Request' when missing email or password", async () => {
    const res = await request(app).post("/api/login").send({
      email: "harry@hogwarts.edu"
    });
